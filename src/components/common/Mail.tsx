...
import { MailDetail } from './MailDetailView';
...
const Mail: React.FC = () => {
    ...
    const fetchInbox = useCallback(async () => {
        ...
        try {
            ...
            // Get current user ID to filter only received mails
            const currentUserId = localStorage.getItem('userId') || sessionStorage.getItem('userId');

            const normalized = Array.isArray(mailsData)
                ? mailsData
                    .filter((mail: any) => {
                        // Only show mails where current user is recipient (not sender)
                        // Inbox should only have mails sent TO current user
    ...
    useEffect(() => {
        ...
        console.log('📨 Mail socket effect', { userId });
        if (!userId) return;
        ...
        try {
            ...
            if (mailDetailId && mail.thread_id) {
                ...
                try {
                    ...
                    // Handle thread response - extract the first mail from mails array
                    let detailData = responseData;
                    if (responseData?.mails && Array.isArray(responseData.mails) && responseData.mails.length > 0) {
                        detailData = responseData.mails[0];
                        // Also preserve replies and thread info
                        if (responseData.mails.length > 1) {
                            detailData.replies = responseData.mails.slice(1);
                        }
                    }
  ...
  const handleSendCompose = async () => {
    ...
    try {
        ...
        if (composeAttachments.length > 0) {
            ...
            formData.append('recipients', JSON.stringify(mailPayload.recipients)); // Backend expects JSON string for recipients
            formData.append('subject', mailPayload.subject);
            formData.append('body', mailPayload.body);
            composeAttachments.forEach((file) => {
                formData.append('attachments', file);  // ✅ Fixed: All files under "attachments"
            });
        ...
        if (view === 'sent') await fetchSentMails(); // Refresh sent mails if on that view
    } catch (error: any) {
                            ...
                            <SheetContent side="right" className="w-full sm:max-w-[90vw] md:max-w-[80vw] lg:max-w-[75vw] xl:max-w-[70vw] p-0">
                                {selectedMail && (
                                    <MailDetail
                                        ...
                                        onDelete={(mailId) =>
                                            handleDeleteMail(String(selectedMail.thread_id ?? mailId))
                                        }
