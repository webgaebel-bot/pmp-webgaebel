...
//       Object.entries(current).forEach(([rowId, draftValues]) => {
//         const row = rows.find((item) => item.id === rowId);
//         if (!row) return;

//         const draftKeys = Object.keys(draftValues);
//         const rowKeys = Object.keys(row.values || {});
//         const allKeys = new Set([...draftKeys, ...rowKeys]);
//         const matches = [...allKeys].every((key) => {
//           const left = String(draftValues[key] ?? '').trim();
//           const right = String(row.values?.[key] ?? '').trim();
//           return left === right;
//         });
...
import React, { useMemo, useState } from 'react';
import { AlertTriangle, Check, Maximize2, Minimize2, PencilLine, Plus, Trash2, X } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Badge } from '@/components/ui/badge';
import type { FlexibleColumn } from '@/types/leads';
...
export function FlexibleSheetTable({
  ...
  // ─── FIX: store a snapshot of (values, raw) at the moment autosave is
  //         queued so that a re-render / index shift cannot corrupt it ──────
  const autoSaveSnapshots = React.useRef<
    Record<string, { rowId: string; values: Record<string, string>; raw: unknown }>
  >({});
  ...
  // Clear draft state when server data already reflects what we typed
  React.useEffect(() => {
    setDraftRows((current) => {
      let changed = false;
      const next = { ...current };
  ...
  const handleScroll = (event: React.UIEvent<HTMLDivElement>) => {
    ...
    if (nearBottom && visibleRowLimit < rows.length) {
      setVisibleRowLimit((current) =>
        Math.min(current + renderBatchSize, rows.length)
      );
    }
  };

  const getValue = (row: FlexibleSheetRow, columnId: string) =>
    draftRows[row.id]?.[columnId] ?? row.values[columnId] ?? '';

  const isRowEditable = (row: FlexibleSheetRow) => !row.raw || Boolean(editingRows[row.id]);
  ...
  const cancelEditingRow = (row: FlexibleSheetRow) => {
    setEditingRows((current) => {
      if (!current[row.id]) return current;
      const next = { ...current };
      delete next[row.id];
  ...
  // ─── Core save: uses the snapshot stored at queue-time, NOT live row state
  const saveValues = async (
    ...
    if (savingRows.current[rowId]) {
      queuedSaveValues.current[rowId] = values;
      return;
    }

    savingRows.current[rowId] = true;
    try {
      await onSaveRow(rowId, values, raw);

      // Only clear the draft if this row has no server-backed raw data yet
      // (blank row). For existing rows the useEffect above handles cleanup.
      if (!raw) {
        setDraftRows((current) => {
      ...
      savingRows.current[rowId] = false;
      delete autoSaveSnapshots.current[rowId];

      const queuedValues = queuedSaveValues.current[rowId];
      delete queuedSaveValues.current[rowId];
      if (queuedValues) {
        // raw may have been updated; fetch from live rows
        const liveRaw = rows.find((r) => r.id === rowId)?.raw ?? raw;
        void saveValues(rowId, queuedValues, liveRaw);
      }
    }
  };

  // ─── FIX: snapshot row id + values + raw at the moment the timer fires ──
  const queueAutoSave = (
    rowId: string,
    values: Record<string, string>,
    raw: unknown
    ...
    // Always update the snapshot so the timer uses the latest typed value
    autoSaveSnapshots.current[rowId] = { rowId, values, raw };

    window.clearTimeout(autoSaveTimers.current[rowId]);
    autoSaveTimers.current[rowId] = window.setTimeout(() => {
      const snapshot = autoSaveSnapshots.current[rowId];
      if (!snapshot) return;
      void saveValues(snapshot.rowId, snapshot.values, snapshot.raw);
    }, autoSaveDelay);
  };

  React.useEffect(() => {
    return () => {
      Object.values(autoSaveTimers.current).forEach((timerId) =>
  ...
  const setValue = (
    ...
    // ─── FIX: pass raw snapshot so the timer knows which record to update ──
    queueAutoSave(rowId, nextValues, raw);
  };

  const saveRow = (row: FlexibleSheetRow) => {
    if (!onSaveRow) return;
    const values = { ...row.values, ...(draftRows[row.id] || {}) };
    void saveValues(row.id, values, row.raw)
      .then(() => {
        setEditingRows((current) => {
          if (!current[row.id]) return current;
          const next = { ...current };
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Cells accept multiple values with comma, semicolon, pipe, or new line.
          </p>
        </div>
        {canManageColumns || canAdd ? (
          <div className="flex flex-col gap-2 sm:flex-row">
      ...
      <div
        className={
          isFullscreen
            ? 'max-h-[calc(100vh-14rem)] overflow-auto rounded-lg border'
      ...
      >
        <table className="w-full min-w-[1600px] border-collapse text-sm">
          <thead className="bg-muted/60">
            <tr>
              <th className="sticky left-0 z-20 w-14 border-r bg-muted/90 px-3 py-2 text-center font-medium">
                #
              </th>
              {showOwner ? (
                <th className="sticky left-14 z-10 w-44 border-r bg-muted/90 px-3 py-2 text-left font-medium">
                  {ownerLabel}
                </th>
              ) : null}
              {visibleColumns.map((column) => (
                <th
                  key={column.id}
                  className="min-w-[240px] border-r px-2 py-2 text-left align-top font-medium"
                >
                  <div className="flex items-center gap-2">
                    {canManageColumns ? (
                      <Input
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr>
                <td
                  colSpan={visibleColumns.length + (showOwner ? 3 : 2)}
                  ...
                  <tr key={row.id} className="border-t hover:bg-muted/30">
                          ...
                          >
                            ...
                            <textarea
                              ...
                              onBlur={() => {
                                if (!autoSave || !canEdit || !onSaveRow) return;
                                window.clearTimeout(
                                  autoSaveTimers.current[row.id]
                                );
                              const snapshot =
                                autoSaveSnapshots.current[row.id];
                              if (snapshot) {
                                void saveValues(
                                  snapshot.rowId,
                                  snapshot.values,
                                ...
                                // No pending draft — nothing to flush
                              }
                              }}
                            onKeyDown={(event) => {
                              if (
                    ...
                    <td className="p-2 align-top">
                      <div className="flex flex-wrap gap-1">
                        {canEdit && onSaveRow && row.raw && !editable ? (
                          <Button
                            type="button"
                            size="icon"
                            variant="ghost"
                        ...
                        {canEdit && onSaveRow && editable && row.raw ? (
                          <Button
                            type="button"
                            size="icon"
                            variant="ghost"
                            onClick={() => cancelEditingRow(row)}
                            title="Cancel edit"
                          >
                            <X className="h-4 w-4" />
                          </Button>
            ...
            {visibleRowLimit < rows.length ? (
              <tr>
                ...
                >
                  Scroll down to load more rows ({visibleRowLimit} of{' '}
                  {rows.length})
                </td>
              </tr>
            ) : null}
